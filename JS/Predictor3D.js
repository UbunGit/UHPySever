...
$(function() {
	recommendDataResult = null;// 统计数据
	frequencyDataResult = null;// 彩票某一期出球频率数据
	// 获取频率统计数据
	function getData(type, count, beginData, enddate) {
		...
		$.ajax({
			...
			success : function(data) {
				hidddle_loading();
				if (data.inforCode == 0) {
					recommendDataResult = data.result;
				} else {
					var msg = data.result;
					show_err_msg(msg);
				}
		...
		return recommendDataResult;
	}
	// 刷新频率统计
	function reloadBar() {
		var count = $(".probabilityCount").find("option:selected").text();
	...
	$(".probabilityCount").change(function(e) {
	...
	var myDate = new Date();
	$("#enddate").val(myDate.toLocaleDateString());
	reloadBar();
